...
const actions = {
  weaponPrimaryFired: assign((context, action) => {
    ...
    bullet.setVelocity(
      Math.cos(radianFacing) *
        context.ship.__customAdditions__.SHIP_SPECS.weapon.absoluteVelocity +
        context.ship.body.velocity.x,
  ...
  weaponsCharge: assign((context, action) => {
    ...
    const fpLevel =
      action.delta /
      context.ship.__customAdditions__.SHIP_SPECS.weapon.chargeDamper;
...
function createWeaponChargeLevelMeter(ship) {
  ...
  weaponChargeLevelMeter.setAttribute(
    "low",
    ship.__customAdditions__.SHIP_SPECS.weapon.cost
  );
