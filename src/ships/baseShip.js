...
export function baseShip({ shipIndex = 0, createContext, worldLayer }) {
  ...
  this.ship = createContext.physics.add.sprite(
    window.innerWidth / 2,
    window.innerHeight / 2,
    "ships",
    this.SHIP_SPECS.frame.startIndex
  );
...
baseShip.prototype.createWeaponChargeLevelMeter = function() {
  ...
  document
    ...
    .appendChild(weaponChargeContainer);
};
...
baseShip.prototype.createThrustBoostLevelMeter = function() {
  ...
  document
    .querySelector(".game-meta-information")
    .appendChild(thrustBoostContainer);
};
...
baseShip.prototype.handleRotationInput = function({ cursors, time }) {
  if (cursors.left.isDown || cursors.right.isDown) {
    this.shipState = this.shipStateMachine.transition(this.shipState, {
      type: "ROTATE",
      direction: cursors.left.isDown ? "left" : "right"
    });
  } else {
    this.shipState = this.shipStateMachine.transition(this.shipState, {
      type: "NOROTATE"
...
baseShip.prototype.thrustNormal = function({ cursors, updateContext }) {
  ...
  const defaultOrBoost =
    cursors.shift.isDown && this.thrustBoostCharge > 0
      ? "boostThrust"
      : "thrust";
  ...
  const xAcceleration =
    Math.cos(radianFacing) * baseAcceleration * (cursors.down.isDown ? -1 : 1);
  const yAcceleration =
    Math.sin(radianFacing) * baseAcceleration * (cursors.down.isDown ? 1 : -1);
...
baseShip.prototype.handleThrustInput = function({ cursors }) {
  if (cursors.up.isDown || cursors.down.isDown) {
    ...
    this.shipState = this.shipStateMachine.transition(this.shipState, {
      type,
      direction: cursors.up.isDown ? "forward" : "backward"
    });
  } else {
    this.shipState = this.shipStateMachine.transition(this.shipState, {
      type: "NOTHRUST"
...
baseShip.prototype.weaponPrimaryFired = function({
  updateContext,
  worldLayer
}) {
  ...
  bullet.setVelocity(
    Math.cos(radianFacing) * this.SHIP_SPECS.weapon.absoluteVelocity +
      this.ship.body.velocity.x,
    // you have to make sin negative for y because in cirlces, a positive y is up and negative y is down, whereas the opposite is true for canvas
    -Math.sin(radianFacing) * this.SHIP_SPECS.weapon.absoluteVelocity +
      this.ship.body.velocity.y
  );
  this.weaponCharge.level =
    this.weaponCharge.level - this.SHIP_SPECS.weapon.cost;

  // https://photonstorm.github.io/phaser3-docs/Phaser.Physics.Arcade.Factory.html#collider__anchor
  updateContext.physics.add.collider(
    bullet,
    worldLayer,
    (collidedBullet, collidedWorldLayer) => {
